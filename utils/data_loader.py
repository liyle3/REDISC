...
def dataloader(config):
    ...
    if config.data.data in ['Roman-empire', 'Amazon-ratings', 'Minesweeper', 'Tolokers', 'Questions']:
    ...
    if config.data.data in ['ogbn-products', 'ogbn-arxiv']:
        dataset = PygNodePropPredDataset(name=config.data.data, root='data')
        data = dataset[0]
        ...
        nlabel = config.data.nlabel
        data.y = F.one_hot(data.y, nlabel).float()
        data.y = data.y.squeeze(1)

        test_idx = torch.BoolTensor(data.x.shape[0])
        test_idx[:] = False
        test_idx[split_idx['test']] = True
        train_idx = torch.BoolTensor(data.x.shape[0])
        train_idx[:] = False
        train_idx[split_idx['train']] = True
        val_idx = torch.BoolTensor(data.x.shape[0])
        val_idx[:] = False
        val_idx[split_idx['valid']] = True


        # if config.data.data in ['ogbn-arxiv', 'ogbn-products']:
        if config.data.data in ['ogbn-products']:
            data.idx = torch.arange(0, data.num_nodes, dtype=torch.long)
            return data, data.y.to(device_id), train_idx.to(device_id), val_idx.to(device_id), test_idx.to(device_id)
        else:
            return (data.x.to(device_id), data.y.to(device_id), data.edge_index.to(device_id),
                    train_idx.to(device_id), val_idx.to(device_id), test_idx.to(device_id))
